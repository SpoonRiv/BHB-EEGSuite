...
与 EEG 使用相同的时间窗口，纵轴按各路滤波后计数独立缩放。仅有效更新帧进入波形，
未更新帧不重复绘制；停止、断开或长时间无有效数据时清空显示。
PPG 通过 `/ws/ppg` 连续推送，独立于 EEG LSL 通道和 EEG 滤波链路；波形显示前固定经过
程序内四阶 0.2Hz 至 1kHz 目标带通和 50Hz 陷波（受当前约 100Hz PPG 更新率限制，
高截止实际不超过奈奎斯特频率），不做单位换算，
